...
import com.uet.restaurant.Adapter.MyFoodAdapter;
...
public class FoodListActivity extends AppCompatActivity {
    ...
    private void startSearchFood(String query) {
        ...
        mCompositeDisposable.add(mIRestaurantAPI.searchFood(headers, query, selectedCategory.getId())
    ...
    public void loadFoodListByCategory(FoodListEvent event) {
        ...
        if (event.isSuccess()) {
            ...
            HashMap<String, String> headers = new HashMap<>();
            headers.put("Authorization", Common.buildJWT(Common.API_KEY));
            mCompositeDisposable.add(mIRestaurantAPI.getFoodOfMenu(headers, event.getCategory().getId())
                    .subscribeOn(Schedulers.io())
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(foodModel -> {
